...
@pytest.mark.usefixtures("use_testing_shape")
@pytest.mark.parametrize("tag", [t for t in NODE_EXTENSIONS[0].tags if "/reference_files/" in t.tag_uri])
...
def test_make_pixelarea():
    ...
    assert isinstance(pixelarea.meta.photometry.pixelarea_steradians, u.Quantity)
    assert isinstance(pixelarea.meta.photometry.pixelarea_arcsecsq, u.Quantity)
